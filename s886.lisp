...
(def free-bird-song-started 0)

(defun play-stop ()
    (progn
        (sleep 0.3)
        (foc-play-stop)
        (if (= free-bird-song-started 1)
            (progn
                (setq free-bird-song-started 0)
            )
        )
    )
)
...
(defun read-thd ()
    (loopwhile t {
            ...
            (if (and (= (bufget-u8 uart-buf-rx 0) 1) (check-crc))
                (progn
                    ...
                    (calculate-impulses)
                    (if (and (> speed-x10 350) (= free-bird-song-started 0))
                        (progn
                            (spawn 150 free-bird-song)
                            (setq free-bird-song-started 1)
                        )
                    )
...
;(override-temp-motor 0); Express has a different UART start function, see the documentation for details
(if (eq (sysinfo 'hw-type) 'hw-express)
    (uart-start 1 20 21 9600)
    (uart-start 9600)
)
